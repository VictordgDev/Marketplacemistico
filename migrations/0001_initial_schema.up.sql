...
CREATE TABLE IF NOT EXISTS seller_billing_profiles (
    id SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_products_publicado ON products(publicado);
...
CREATE INDEX IF NOT EXISTS idx_users_phone ON users(telefone);
CREATE INDEX IF NOT EXISTS idx_users_document ON users(cpf_cnpj);
CREATE UNIQUE INDEX IF NOT EXISTS idx_sellers_nome_loja_lower ON sellers(LOWER(nome_loja));
CREATE INDEX IF NOT EXISTS idx_shipping_quotes_cart ON shipping_quotes(cart_id);
CREATE INDEX IF NOT EXISTS idx_shipping_quotes_seller ON shipping_quotes(seller_id);
